...
$DIR/workerScripts/launch-vm.sh

# Step 2: install software in the VM instance.
$DIR/workerScripts/install-worker.sh

$DIR/workerScripts/test-vm.sh

# Step 4: create VM image (AIM).
aws ec2 create-image --instance-id $(cat workerScripts/instance.id) --name Worker-Image | jq -r .ImageId > workerScripts/image.id
echo "New VM image with id $(cat workerScripts/image.id)."
...
aws ec2 terminate-instances --instance-ids $(cat workerScripts/instance.id)
